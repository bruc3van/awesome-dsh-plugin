...
| `data/packages.json` | 维护者 / AI 审核（**手动维护，脚本绝不重写**） | 插件安装目标映射：`"owner/name"`（小写）→ 每个目标含 `install`（**粘贴进官方「添加插件」输入框的准确安装名——npm 包名（含版本）或 Git 仓库地址，`github:` ref 语法要转成 `https://github.com/owner/repo`**）、`command`（终端用的完整 `dsh plugin` 命令，含 runner 前缀与 `#ref`）、`source`（分类前缀）、profile、requirements / tasks / note 与验证状态（`readme-verified` = 命令逐字取自上游 README）。首批 11 条人工精修；2026-09-26 批量回访 feed 全部 600 仓 README 程序化提取至 494 条（自引用优先、歧义与仅本地安装不收录、模板占位符剔除），requirements/tasks 仍待人工补。由 `refresh-market-v2` 工作流投影进 `data/market-v2.json`；不在 feed 里的映射网页目录仍可直接读本文件。`scripts/validate-packages.mjs` 离线校验结构、install 形态、快照与 curation 一致性 |
...
node scripts/market-v2.mjs              # 重建 data/market-v2.json（v1 feed + packages 映射的 v2 超集）
