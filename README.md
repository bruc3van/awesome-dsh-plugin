...
| 把 Claude 项目的配置带进 DSH | [dsh-claude-migrator](https://github.com/huanghouchun/dsh-claude-migrator) | 自动识别用户主目录与项目根的 .claude/skills、.claude/rules、.mcp.json、CLAUDE.md（零拖动），skill 按工作区隔离唤醒、MCP 动态注册真实连接；内置可折叠「CLAUDE 配置中心」看板，适合从 Claude Code 迁移到 DSH 的开发者。 |
