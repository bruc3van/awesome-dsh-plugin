...
| 从 Claude 项目迁移配置到 DSH（skills/rules/MCP 自动识别） | [dsh-claude-migrator](https://www.npmjs.com/package/dsh-claude-migrator) | 用户主目录（~/.agents/skills、~/.claude/skills）与项目根（.claude/、.mcp.json）的 Claude 配置零拖动自动识别；skill 按工作区隔离唤醒（provider 机制），MCP 动态注册真实连接（密钥 ${VAR} 环境变量展开 + 自动重连）；内置可折叠看板（全局/项目来源徽标、连接状态、点击工作区自动切换、主题自适应）。 |
