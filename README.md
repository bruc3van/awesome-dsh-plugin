...
- **[dsh-movein](https://github.com/sjh9714/dsh-movein)**（[@sjh9714](https://github.com/sjh9714) · 2026-08-16）- 预览并将 Claude Code、Codex 与 OpenCode 的受支持配置迁入 DSH。涵盖技能、命令、代理、指令与本地或远程 MCP 服务，支持 OpenCode V1/V2 JSONC 优先级并保留环境变量占位符。无法安全转换的语义会列为未支持项，仍提供目标冲突保护、备份、恢复和 `doctor` 诊断。
