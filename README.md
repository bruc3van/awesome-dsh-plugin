...
- **[dsh-whale-meter](https://github.com/Shiye-10Pages/dsh-whale-meter)**（[@Shiye-10Pages](https://github.com/Shiye-10Pages) · 2026-08-17）— 用量账本与战绩卡：按本月 token 消耗评 🐟→🐳 五档段位，分位由仓库内的分布曲线本地估算（零上报，UI 标注为估算而非全球排名）；6 家厂商 46 个模型逐项核对官方定价页，支持国内厂商按输入长度分档（GLM-5.1 输入 <32K 走 6 元、≥32K 走 8 元）；自动回填安装前的会话；价目按生效区间组织，8·17 调价前后各按各价。适合想知道自己到底烧了多少、又不想把用量数据交出去的人。
