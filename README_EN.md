...
- **[dsh-claude-migrator](https://github.com/huanghouchun/dsh-claude-migrator)** ([@huanghouchun](https://github.com/huanghouchun) · 2026-08-15) — Claude → DSH config migration plugin (v0.2.0): auto-discovers Claude configs (`.claude/skills`, `.claude/rules`, `.mcp.json`, `CLAUDE.md`) from your user home and project root with zero dragging; skills wake per-workspace via a provider mechanism (project-level `.claude/skills` work without cross-project leakage), rules auto-convert to skills (paths kept in whenToUse); MCP servers dynamically register as real connections (env-var `${VAR}` expansion + auto-reconnect); built-in collapsible dashboard (global/project source badges, connection status, auto-switch on workspace click, theme-aware). Install: `dsh plugin --profile web add dsh-claude-migrator`.
