...
| Bring Claude project configs into DSH | [dsh-claude-migrator](https://github.com/huanghouchun/dsh-claude-migrator) | Auto-discovers `.claude/skills`, `.claude/rules`, `.mcp.json`, `CLAUDE.md` from your user home and project root (zero dragging); skills wake per-workspace, MCP servers register as real connections; built-in collapsible "CLAUDE Config Center" dashboard — ideal for developers migrating from Claude Code to DSH. |
