...
- **[dsh-git-status](https://github.com/Wongzexu/dsh-git-status)** ([@Wongzexu](https://github.com/Wongzexu) · 2026-08-15) — A Git status (Git Graph) plugin: a drawer on the right edge of the DSH web UI with a commit DAG lane graph, uncommitted-changes/stash virtual rows and inline diffs, plus right-click branch/tag actions — switch, merge, rename, delete, create — and one-click fetch from all remotes; a complete workbench focused on Git branch and status handling.
